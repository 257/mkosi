...
class Gentoo:
    ...
    EBUILD_SH_ENV_DIR: str
    GLOBAL_CONFIG_PATH: str
    PORTAGE_PACKAGE_ATOM: str
    PROFILE_PATH: str
    USER_CONFIG_PATH: str

    def __init__(self, args, root, do_run_build_script):
        ...
        try:
            ...
            self.EBUILD_SH_ENV_DIR = EBUILD_SH_ENV_DIR
            self.GLOBAL_CONFIG_PATH = GLOBAL_CONFIG_PATH
            self.PORTAGE_PACKAGE_ATOM = PORTAGE_PACKAGE_ATOM
        ...
        if args.architecture:
            self.arch, self.kimg_path = self.ARCHITECTURES[args.architecture]
        else:
            self.arch, self.kimg_path = self.ARCHITECTURES["x86_64"]

        # GENTOO_UPSTREAM: we only support systemd profiles! and only no-multilib for now;
        ...
        self.pkgs_sys = {"@system"}
        # don't rely on default flags from upstream, explicitly emerge *our* systemd
        self.pkgs_sys.add("systemd")
        ...
        if args.ssh:
            self.pkgs_sys.add("net-misc/openssh")
    ...
    def set_profile(self, args: CommandLineArguments, root: str) -> None:
        os.makedirs(os.path.join(root, self.CUSTOM_PROFILE_PATH), exist_ok=True)
        with open(os.path.join(root, self.CUSTOM_PROFILE_PATH, "use.mask"), "w") as f:
            f.write(
                dedent(
                    """\
                    split-bin
        ...
        profile_path = os.path.join(root, self.PROFILE_PATH)
        if not os.path.islink(profile_path):
            MkosiPrinter.print_step(f"{args.distribution} setting Profile")
            os.makedirs(os.path.join(root, self.USER_CONFIG_PATH), exist_ok=True)
    ...
    def fetch_fix_stage3(self, root: str) -> None:
        ...
        with urllib.request.urlopen(stage3tsf_path_url) as r:
            ...
            # out of the loop, check if we've got it.
            if not stage3_tar:
    ...
    def baselayout(self, args: CommandLineArguments, root: str) -> None:
        ...
        self.invoke_emerge(args, root, inside_stage3=False, pkgs={"sys-apps/baselayout"})
        self.portage_use_flags.remove("build")
        os.environ["USE"] = " ".join(self.portage_use_flags)
...
def install_gentoo(args: CommandLineArguments, root: str, do_run_build_script: bool) -> None:
    # IMPROVEME: kimg_path is the only user of gentoo outside of install_getoo
    global gentoo
    gentoo = Gentoo(args, root, do_run_build_script)

    package_accept_keywords = os.path.join(root, gentoo.USER_CONFIG_PATH, "package.accept_keywords")
    ...
    with open(os.path.join(package_accept_keywords, "mkosi"), "a") as f:
        ...
        f.write(
            dedent(
                f"""\
                sys-auth/pambase ~{gentoo.arch}
                sys-kernel/gentoo-kernel-bin ~{gentoo.arch}
                virtual/dist-kernel ~{gentoo.arch}
                """
            )
        )

    package_license = os.path.join(root, gentoo.USER_CONFIG_PATH, "package.license")
    ...
    with complete_step(f"{args.distribution}: patching dracut [https://bugs.gentoo.org/765208]"):
        patchdir = os.path.join(root, gentoo.USER_CONFIG_PATH, "patch")
        ...
        with open(os.path.join(package_accept_keywords, "bug765208"), "a") as f:
            f.write(dedent(f"""={dracut_atom}-053 ~{gentoo.arch}"""))

    os.makedirs(os.path.join(root, gentoo.USER_CONFIG_PATH, "repos.conf"), exist_ok=True)
    with open(os.path.join(root, gentoo.USER_CONFIG_PATH, "repos.conf", "eselect-repo.conf"), "w") as f:
        f.write(
            dedent(
                f"""\
                [gentoo]
                location = {gentoo.portage_cfg["PORTDIR"]}
    ...
    # useflags
    package_use = os.path.join(root, gentoo.USER_CONFIG_PATH, "package.use")
    ...
    with open(os.path.join(package_use, "grub"), "a") as f:
        # repart for usronly
        f.write(
            dedent(
                f"""\
    ...
    # gentoo_mkosi_conf
    package_env = os.path.join(root, gentoo.USER_CONFIG_PATH, "package.env")
    ebuild_sh_env_dir = os.path.join(root, gentoo.EBUILD_SH_ENV_DIR)
    ...
    uninstall_ignore = 'UNINSTALL_IGNORE="${UNINSTALL_IGNORE} ' + " ".join(gentoo.UNINSTALL_IGNORE) + '"'
    use = 'USE="${USE} ' + " ".join(gentoo.portage_use_flags) + '"'
    ...
    # END gentoo_mkosi_conf

    if "build-script" in ARG_DEBUG:
        gentoo.invoke_emerge(args, root, actions={"--info"})

    # avoiding roundabout dependencies
    # homed:
    # portage_use_flags.remove("systemd")
    # portage_use_flags.add("-systemd")
    # gentoo.invoke_emerge(args, root, pkgs={"sys-fs/lvm2"})
    # portage_use_flags.remove("-systemd")
    # portage_use_flags.add("systemd")
    # gentoo.invoke_emerge(args, root, pkgs={"virtual/libudev"})

    # baselayout gets remerged here because of USE=build which overrides
    # /etc/os-release; (see below)
    gentoo.invoke_emerge(args, root, pkgs=gentoo.pkgs_sys, opts=gentoo.EMERGE_UPDATE_OPTS)

    if gentoo.pkgs_fs:
        gentoo.invoke_emerge(args, root, pkgs=gentoo.pkgs_fs)

    # GENTOO_TOREMOVE: https://bugs.gentoo.org/788190
    # GENTOO_DONTMOVE: baselayout is settled down by now, dracut needs this for
    ...
    if not do_run_build_script and args.bootable:
        ...
        configure_dracut(args, root)
        for p in gentoo.pkgs_boot:
            gentoo.invoke_emerge(args, root, pkgs=p)
        if args.esp_partno:
            p = {"sys-kernel/gentoo-kernel-bin"}
            gentoo.invoke_emerge(args, root, pkgs=p, actions={"--config"})

    if args.packages:
        gentoo.invoke_emerge(args, root, pkgs=set(args.packages))

    # DEBUG: investigate if sysvinit flag for systemd has an effect on this
...
def run_postinst_script(
    ...
    with mount_cache(args, root), complete_step("Running postinstall script"):
        ...
        # GENTOO: we tolerate this; system is bootable and these extra packages
        # users can be merged later on manually
        if result.returncode != 0:
            MkosiPrinter.warn(f"postinst returned non-zero exit code {result.returncode}.")
        ...
        with os.scandir(os.path.join(root, "lib/modules")) as d:
            for kver in d:
                ...
                if args.distribution == Distribution.gentoo:
                    cmdline[4] = f"/usr/src/linux-{kver.name}/{gentoo.kimg_path}"
