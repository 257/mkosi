...
class Installer(DistributionInstaller):
    ...
    def install(cls, context: Context) -> None:
        pkguse_dir = Emerge.stage3 / "etc/portage/package.use"
        pkguse_dir.mkdir(parents=True, exist_ok=True)
        with (pkguse_dir / "build").open("w") as f:
            f.write("sys-apps/baselayout build")
        cls.install_packages(context, [ "sys-apps/baselayout" ])
        (pkguse_dir / "build").unlink()

    @classmethod
    def install_packages(cls, context: Context, packages: Sequence[str]) -> None:
        Emerge.invoke(context, arguments=packages)
