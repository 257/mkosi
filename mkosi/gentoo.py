...
class Gentoo:
    ...
    def try_import_portage(self, root: Path, dry_run: bool) -> None:
        try:
            ...
            MkosiPrinter.warn(f"{self.NEED_PORTAGE_MSG}")
            MkosiPrinter.info(self.PORTAGE_INSTALL_INSTRUCTIONS)
            raise MkosiException(e)

        if dry_run:
            return

        self.profile_path = root / PROFILE_PATH
        self.custom_profile_path = root / CUSTOM_PROFILE_PATH
        self.ebuild_sh_env_dir = root / EBUILD_SH_ENV_DIR
        self.portage_cfg_dir = root / USER_CONFIG_PATH
    ...
    def __init__(
        ...
        self.try_import_portage(root, dry_run=False)
    ...
    def invoke_emerge(
        ...
        if not inside_stage3:
            ...
            run_workspace_command(
                ...
                nspawn_params=self.DEFAULT_NSPAWN_PARAMS + ["--bind=/sys"],
            )
